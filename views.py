...
def home(request):
  ...
  me1 = graph.fql("SELECT current_location FROM user WHERE uid IN (SELECT uid2 FROM friend WHERE uid1 = me())")
  ...
  pic = graph.fql("SELECT pic_square FROM user WHERE uid = me() OR uid IN (SELECT uid2 FROM friend WHERE uid1 = me())")
  pics = list()
  for lis in pic:
    s = lis.get('pic_square')
    if s is not None:  
       pics.append(s.encode('ascii','ignore'))
  ...
  for li in me1:
    ...
    if s is not None:
       ...
       g.append(t)  
  g = json.dumps(g)
  return render(request, 'hellodjango/templates/map.html', {'me':me,'me1':me1,'locations':g,'pic':pics})
